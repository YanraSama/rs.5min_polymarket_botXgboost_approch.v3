    ...
    while True:
        try:
            ...
            if new_period == True:
                ...
                send_telegram(
                f"""
                Entrainement modele classifier 3.4: 
...
def backtest(seuil):
    ...
    for th in [0.75, 0.70, 0.65, 0.60]:
        ...
        if "dd_max" in df.columns:
            df_filtered = df_filtered_3[df_filtered_3["dd_max"] < 0.30].copy()
        else:
            print("⚠️ Colonne dd_max introuvable, aucune filtration appliquée")
            df_filtered = df_filtered_3.copy()
            
        if len(df_filtered) == 0:
            print(f"❌ Aucune combinaison avec win_rate > {th}")
            if th == 0.60:
...
def filtre_horaire():
    try:
        now = datetime.now()
        return True
        if int(now.strftime("%H")) >= 7 and int(now.strftime("%H")) < 23:
...
no_trade_took = {"token" : 0, "yes_token" : 0, "no_token" : 0, "trade": False, "cout": 0, "time" : 0,"date" : 0, "heure_entry" : 0, "heure_exit" : 0,"side" : "", "prix_exec" : 0, 
                 "temps" : 0, "resultat" : 0, "price" : 0, "variation" : 0, "profit" : 0, "stake" : 0, "ask_entry" : 0, "bid_exit" : 0, "ratio_entry" : 0, "moyenne_ratio_entry" : 0, "tendance_entry" : 0, "slope_entry" : 0,
                 "ratio" : 0, "imbalance" : 0, "moyenne_ratio" : 0, "tendance" : 0, "slope" : 0, "delta_imbalance" : 0, "whale" : 0, "proba" : 0, "gain" : 0, "ttp" : 0, "dd" : 0, "PM" : 0, "benef" : 0, "ev" : 0, "ttp_dd" : 0, "proba_entry": 0.0,}
       ...
       #------------------------------------------------------------------------------------------------------------------------------------------
        if window != last_window:
            ...
            if trade_took ["trade"] == True:
                ...
                send_telegram(
                    f"""
                🛑 POSITION CLOTURE - Convergence - order_flow_v3.4 : 
            ...
            if no_trade_took ["trade"] == True:
                
                if no_trade_took["side"] == "BUY" and yes_bid > 0.9:
                    no_trade_took["resultat"] = no_trade_took["profit"]
                elif no_trade_took["side"] == "BUY" and yes_bid < 0.1:
                    no_trade_took["resultat"] = -no_trade_took["cout"]
     
                if no_trade_took["side"] == "SELL" and no_bid > 0.9:
                    no_trade_took["resultat"] = no_trade_took["profit"]
                elif no_trade_took["side"] == "SELL" and no_bid < 0.1:
                    no_trade_took["resultat"] = -no_trade_took["cout"]          

                no_trade_took ["heure_exit"]  = now.strftime("%H:%M:%S")  
                no_trade_took["trade"]        = False
                no_trade_took["price"]        = btc_price
                no_trade_took["bid_exit"] = no_bid if no_trade_took["side"] == "SELL" else yes_bid
                no_trade_took["imbalance"] = yes_imbalance if no_trade_took ["side"] == "BUY" else no_imbalance
                no_trade_took["moyenne_ratio"] = moyenne_yes if no_trade_took ["side"] == "BUY" else moyenne_no
                no_trade_took["tendance"] = tendance_yes if no_trade_took ["side"] == "BUY" else tendance_no
                no_trade_took["slope"] = slope_yes if no_trade_took["side"] == "BUY" else slope_no 
                maj_csv(trade_took)# cloture de position
                # print ("retranscription convergence : ", trade_took["resultat"], "$")
...
#---------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        if trade_took["trade"] == True:   
            ...
            if params["STOP_LOSS"] > trade_took ["resultat"] or  proba_drop > params["DROP_LIMIT"] or (trade_took["side"] == "BUY" and (trailing_stop >= yes_bid or yes_bid >= 0.98)) or (trade_took["side"] == "SELL" and (trailing_stop >= no_bid or no_bid >= 0.98)) or (restant < 5 and trade_took ["resultat"] > 0):
            ...
            if trade_took["trade"] == False:
                send_telegram(
                    f"""
                🛑 POSITION CLOTURE order_flow_v3.4 : 
        ...
        if no_trade_took["trade"] == True:   
            ...
            current_proba = proba if (no_trade_took["side"] == "BUY") else proba_no
            
            shares = BUDGET / price_token
            fee_entry = polymarket_taker_fee(shares, price_token)                        
         
            no_trade_took ["resultat"]   = (yes_bid*no_size) - no_trade_took["cout"] - fee_entry if no_trade_took ["side"] == "BUY" else (no_bid*size) - no_trade_took["cout"] - fee_entry
            
            proba_drop = no_trade_took["proba_entry"] - current_proba
            
            if (proba_drop > params["TR_DROP"] or (no_trade_took["side"] == "BUY" and (no_trade_took["resultat"] > 0.0 and  yes_bid >= no_trade_took["ask_entry"] + (params["TL_LEVEL"]*2)))) and trailing_stop < yes_bid-params["TL_LEVEL"]:
                trailing_stop = yes_bid-params["TL_LEVEL"]
                # print ("Trailing stop is moving :", trailing_stop)
            elif (proba_drop > params["TR_DROP"] or (no_trade_took["side"] == "SELL" and (no_trade_took["resultat"] > 0.0 and  no_bid >= no_trade_took["ask_entry"] + (yes_bid-params["TL_LEVEL"]*2)))) and trailing_stop < no_bid-yes_bid-params["TL_LEVEL"]:
                trailing_stop = no_bid-yes_bid-params["TL_LEVEL"]
                # print ("Trailing stop is moving :", trailing_stop)
     
            print ("Trailing SL : ",  trailing_stop, " ", proba_drop > params["TR_DROP"], "   ", (no_trade_took["side"] == "BUY" and (no_trade_took["resultat"] > 0.0 and  yes_bid >= no_trade_took["ask_entry"] + (params["TL_LEVEL"]*2)))
                if no_trade_took["side"] == "BUY" else (no_trade_took["side"] == "SELL" and (no_trade_took["resultat"] > 0.0 and  no_bid >= no_trade_took["ask_entry"] + (params["TL_LEVEL"]*2))), "  ",trailing_stop < yes_bid-params["TL_LEVEL"])
     
            print ("TEST MANA : ",  no_trade_took ["resultat"], "  ", trailing_stop, " ",yes_bid if no_trade_took["side"] == "BUY" else no_bid, "  ",  proba_drop, " ",current_proba, "   ", restant, "s")
                
                
            if params["STOP_LOSS"] >  no_trade_took ["resultat"] or  proba_drop > params["DROP_LIMIT"] or (no_trade_took["side"] == "BUY" and (trailing_stop >= yes_bid or yes_bid >= 0.98)) or (no_trade_took["side"] == "SELL" and (trailing_stop >= no_bid or no_bid >= 0.98)) or (restant < 5 and trade_took ["resultat"] > 0):
               
                # if cloturer_position(no_trade_took, no_trade_took["token"], size) == None :
                    # continue 
                        
                no_trade_took ["price"] = btc_price 
                no_trade_took ["heure_exit"] = now.strftime("%H:%M:%S")  
                no_trade_took["ratio"] = ratio_buy if no_trade_took["side"] == "BUY" else ratio_sell
                # no_trade_took ["resultat"]   = (yes_bid*size) - no_trade_took["cout"] if no_trade_took ["side"] == "BUY" else (no_bid*size) - no_trade_took["cout"]
                no_trade_took["moyenne_ratio"] = moyenne_yes if no_trade_took ["side"] == "BUY" else moyenne_no
                no_trade_took["tendance"] = tendance_yes if no_trade_took ["side"] == "BUY" else tendance_no
                no_trade_took["bid_exit"] = no_bid if no_trade_took["side"] == "SELL" else yes_bid
                no_trade_took["slope"] = slope_yes if no_trade_took["side"] == "BUY" else slope_no 
                ...
                state_trade_lvl = 0
                maj_csv(no_trade_took)                                     # cloture de position
                no_trade_took["trade"] = False                  
                # print( "temps dépassé : cloture de position et retransciption ...") 
                print ("Condition cloture : ", no_trade_took["resultat"] , "$  ", restant, "   ", no_trade_took["side"], "   ", yes_bid if no_trade_took ["side"] == "BUY" else no_bid, "    ", no_trade_took["cout"]) 
                
            if no_trade_took["trade"] == False:
                send_telegram(
                    f"""
                🛑 POSITION CLOTURE order_flow_v3.4 : 

                    Side      : {no_trade_took['side']}
                    BTC Price : {btc_price}
                    Size      : {no_size}

                💰 Token    : {yes_bid if no_trade_took["side"] == "BUY" else no_bid:.2f} 
                💰 Coût     : {no_trade_took['cout']:.2f}
                🎯 Resultat : {no_trade_took['resultat']:.2f}
                   Restant  :  {restant}
                """
                ) 
        ...
        if trade_took["trade"] == True and no_trade_took["trade"] == True:
            time.sleep(1)
            continue
        ...
        if  trade_took["trade"] == False and yes_ask >  params["MIN_ASK"] and proba > params["PROBA"] and yes_ask < params["MAX_ASK"] and restant < params["LIMIT_TIME_MAX"] and restant > params["LIMIT_TIME_MIN"] and yes_imbalance > params["IMBALANCE"]:
            ...
            send_telegram(
                f"""
            📈 POSITION OUVERTE order_flow_v3.4 - (scalping)
        ...
        if  no_trade_took["trade"] == False and no_ask > params["MIN_ASK"] and proba_no > params["PROBA"] and no_ask < params["MAX_ASK"] and restant < params["LIMIT_TIME_MAX"] and restant > params["LIMIT_TIME_MIN"] and no_imbalance > params["IMBALANCE"]:
            
            no_trade_took ["trade"]           = True
            no_trade_took ["prix_exec"]       = btc_price
            no_trade_took ["date"]            = now.strftime("%d/%m/%Y") 
            no_trade_took ["heure_entry"]     = now.strftime("%H:%M:%S")  
            no_trade_took ["yes_token"]       = yes_token
            no_trade_took ["no_token"]        = no_token
            no_trade_took ["token"]           = no_token  
            no_trade_took ["side"]            = "SELL"  
            no_trade_took ["time"]            = restant
            no_trade_took ["variation"]       = save_variation  
            no_trade_took ["imbalance"]       = no_imbalance
            no_trade_took ["delta_imbalance"] = delta_no_imbalance
            no_trade_took ["proba_entry"]     = proba_no
            no_trade_took ["PM"]              = benef_no
            no_trade_took ["dd"]              = dd_no
            no_trade_took["stake"]            = stake
            save_signal                       = 0
            price_token                       = no_ask
            spread                            = no_spread
            ...
            price_token = yes_ask if no_trade_took["side"] == "BUY" else no_ask
            
            executed_price = yes_ask if no_trade_took["side"] == "BUY" else no_ask  # par défaut
            if response and "avgPrice" in response:
                executed_price = float(response.get("avgPrice", executed_price))
            elif response and "price" in response:
                executed_price = float(response.get("price", executed_price))
            ...
            if True: #details == None:
                no_trade_took ["cout"] = BUDGET + spread
            else:
                no_trade_took ["cout"] = round(float(details["size_matched"]) * executed_price + spread, 2)
            
            no_trade_took["profit"]    = (1 / executed_price * no_trade_took ["cout"]) - no_trade_took ["cout"]
            
            no_size =  round(no_trade_took["cout"] / executed_price, 2)
            
            no_trade_took["ask_entry"] = no_ask if no_trade_took["side"] == "SELL" else yes_ask
            
            no_trade_took["ratio_entry"] = ratio_buy if no_trade_took["side"] == "BUY" else ratio_sell
            
            no_trade_took["temps"] = restant
            
            no_trade_took["moyenne_ratio_entry"] = moyenne_yes if no_trade_took["side"] == "BUY" else moyenne_no 
            no_trade_took["tendance_entry"] = tendance_yes if no_trade_took["side"] == "BUY" else tendance_no
            no_trade_took["slope_entry"] = slope_yes if no_trade_took["side"] == "BUY" else slope_no   

            print( "Analyse de volumme' : ", tendance_yes if no_trade_took["side"] == "BUY" else tendance_no, "    ", ratio_buy if no_trade_took["side"] == "BUY" else ratio_sell, " slope : ", slope_yes if no_trade_took["side"] == "BUY" else slope_no)
        ...
        # if trade_took ["trade"] == True or no_trade_took ["trade"] == True:
