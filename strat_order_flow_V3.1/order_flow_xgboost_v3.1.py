    ...
    if not os.path.exists(CSV_FILE):
        with open(CSV_FILE, "w", newline="", encoding="utf-8-sig") as f:
            ...
            writer.writerow([
                ...
                "stake", "prix_exec", "price", "imbalance", "delta_imbalance", "variation", "ask_entry", "ratio_entry", "moyenne_ratio_entry", "tendance_entry", "slope_entry", "whale", "resultat", "BENEF", "DD", "PM","EV", "TTP", "TTP_DD", "target", "proba_entry"
...
def maj_csv(trade_took):
    ...
    with open(CSV_FILE, "a", newline="", encoding="utf-8-sig") as f:
        ...
        writer.writerow([
            ...
            trade_took["dd"],  trade_took["PM"],  trade_took["ev"], trade_took["ttp"], trade_took["ttp_dd"], 1 if trade_took["resultat"] > 0 else 0, trade_took["proba_entry"]
        ])
    ...
    while True:
        try:    
            ...
            if new_period == True:
                ...
                a, b, void_lign = classifier_target_vectorized(CSV_LIVE)
                df = pd.read_csv(CSV_LIVE, sep=";", encoding="utf-8-sig", low_memory=False)
                ...
                auccurancy, aucc, confusion, importances = learning_data_classifier(CSV_LIVE, "xgboost_model_v3_target.pkl")
                # if void_lign == True:
            ...
            if yes_ask > 0.00 and no_ask > 0.00 and btc_price > 0.00 and stake > 0.00:
...
def watchdog():
    global ws_thread, poly_ws_thread, log_trade_data_thread
...
#--------------------------------------------------------------------------------------------------------------------------
        ...
        proba = regressor_proba.predict_proba(features_yes)[0][1]
        ...
        proba_no = regressor_proba.predict_proba(features_no)[0][1]
...
#---------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        if trade_took["trade"] == True:   
            ...
            elif (proba_drop > TR_DROP or (trade_took["side"] == "SELL" and (trade_took["resultat"] > 0.0 and  no_bid >= trade_took["ask_entry"] + (TL_LEVEL*2)))) and trailing_stop < no_bid-TL_LEVEL:
                trailing_stop = no_bid-TL_LEVEL
                # print ("Trailing stop is moving :", trailing_stop)
     
            print ("Trailing SL : ",  trailing_stop, " ", proba_drop > TR_DROP, "   ", (trade_took["side"] == "BUY" and (trade_took["resultat"] > 0.0 and  yes_bid >= trade_took["ask_entry"] + (TL_LEVEL*2)))
                if trade_took["side"] == "BUY" else (trade_took["side"] == "SELL" and (trade_took["resultat"] > 0.0 and  no_bid >= trade_took["ask_entry"] + (TL_LEVEL*2))), "  ",trailing_stop < yes_bid-TL_LEVEL)
            ...
            if proba_drop > DROP_LIMIT or (trade_took["side"] == "BUY" and (trailing_stop >= yes_bid or yes_bid >= 0.98)) or (trade_took["side"] == "SELL" and (trailing_stop >= no_bid or no_bid >= 0.98)) or (restant < 30 and trade_took ["resultat"] > 0):
        ...
        if yes_ask  > 0:
            EV = proba * ((1-yes_ask )/yes_ask ) - (1- proba)
        else:
            EV = -1    
            
        print ("HAUT Probas XBOOST  TS : ", yes_ask, "$  ",  proba," imba : ", yes_imbalance, " ", yes_ask > MIN_ASK, " ", proba > PROBA, "    " ,yes_ask < MAX_ASK, "  ", restant, "s") 
        
        if EV > STOP_LOSS and yes_ask > MIN_ASK and proba > PROBA and yes_ask < MAX_ASK and restant < LIMIT_TIME_MAX and restant > LIMIT_TIME_MIN:
        ...
        print ("BAS Probas XBOOST  TS : ", no_ask, "$  ",  proba_no," imba : ", no_imbalance,  "    ", no_ask > MIN_ASK , " ",   proba_no > PROBA, " ", no_ask < MAX_ASK, "  ", restant, "s")
        
        rr = benef_no / dd_no if benef_no > 0 and dd_no != 0 else 0.0

        if no_ask  > 0:
            EV_no = proba_no * ((1-no_ask )/no_ask ) - (1- proba_no)
        else:
            EV_no = -1  

        if EV_no > STOP_LOSS and no_ask > MIN_ASK and proba_no > PROBA and no_ask < MAX_ASK and restant < LIMIT_TIME_MAX and restant > LIMIT_TIME_MIN:
