...
def _backtest_core(ask, bid, restant, side, imbalance,
    ...
    while i < N:
        ...
        if ep > 0:
            EV = entry_signal[i] * ((1-ep)/ep) - (1- entry_signal[i])
        else:
            EV = -1
...
_backtest_core(
    ...
    30.0, 120.0, 0.65, 0.32, 0.82, 0.0, 0.0, 0.10, 0.5, -0.2
)
print("✅ Numba compilé")
...
with open("opti_results.csv", "w", newline="", encoding="utf-8-sig") as f:
    ...
    writer.writerow([
        ...
        "proba", "TR SL", "TR DROP", "TR LIMIT_DROP", "RR", "SL"
    ])
